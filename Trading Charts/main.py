...
def get_data_from_api(key, func: str, from_symbol: str, to_symbol: str, outputsize: str):
...
heiken_ashi_df = pd.DataFrame({'Open': ha_open, 'High': ha_high, 'Low': ha_low, 'Close': ha_close})
...
ap_histogram = mpf.make_addplot(histogram, type='bar', panel=1, ylabel='Histogram', color=hist_colors)

# Plotting the candlestick chart
mpf.plot(heiken_ashi_df, type='candle', style='yahoo', mav=[10, 20, 50, 100], ylabel='Exchange Rate', title="AUD/USD exchange ratio", addplot=[ap_macd, ap_signal, ap_histogram, atr_plot])
